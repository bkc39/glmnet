...
- **Plots.** `glmnet/plot` (#28) draws R's `plot.glmnet` and `plot.cv.glmnet` as 4.1.10's `plotCoef`, `plot.multnet`, `plot.mrelnet` and `plot.cv.glmnet` draw them:
  ...
  - Where the plots differ from R's (tick positions, the labels along the top of the CV plot, labels that R clips, one stacked picture instead of one plot per class) is listed in the manual's *Plots* chapter.
